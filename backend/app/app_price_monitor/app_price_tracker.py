...
def app_price_push_generator():
    ...
    app_push_list = PushList(APP_WIDGET_ID).push_list_get().push_list
    for app_push_data in app_push_list:
        push_data = PushData(app_push_data)
        ...
        if content != '':
            if push_data.notify_method == 1:
                address = User(user_id=user_id).wechat_key
            elif push_data.notify_method == 2:
                address = User(user_id=user_id).email
            title = 'App Discount!'
            if (push_data.add_to_push_queue(title, address, content)):
