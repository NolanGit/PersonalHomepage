...
def get():
    try:
        ...
        for file in files:
            ...
            temp[file] = json.load(file_path)

        r.append({'百度': [temp.pop('baidu_now.json'), temp.pop('baidu_today.json'), temp.pop('baidu_now.json')]})
        r.append({'什么': [temp.pop('smzdm_article_today.json'), temp.pop('smzdm_article_week.json'), temp.pop('smzdm_article_month.json')]})
