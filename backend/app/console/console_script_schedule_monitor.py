...
def schedule_get():
    console_script_schedule_query = console_script_schedule.select().where(
        (console_script_schedule.is_valid == 1)
        & (console_script_schedule.trigger_time <= datetime.datetime.now())
    ).dicts()
    ...
    try:
        for schedule in schedules:
            if schedule['is_valid'] == 1:
                console_script_schedule.update(is_valid=2).where(
                    console_script_schedule.id == schedule['id']).execute()

                #记录运行次数
                script_id = schedule['script_id']
                console_script_query = console_script.select().where(
                    (console_script.is_valid == 1)
                    & (console_script.id == script_id)).dicts()
                ...
                console_script_log.create(
                    ...
                    output=schedule['command'] + '<br>' +
                    str(output).replace('\n', '<br>').replace(' ', '&nbsp;'),
                    user=schedule['user'] + '(定时)',
                    start_time=datetime.datetime.now())
                console_script_schedule.update(is_valid=0).where(
                    console_script_schedule.id == schedule['id']).execute()
...
def generate_next_schedule(schedule):
    if schedule['is_automatic'] == 1:
        console_script_schedule.create(
            ...
            datetime.timedelta(hours=schedule['interval']),
